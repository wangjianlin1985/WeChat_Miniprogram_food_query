...
public class NoticeController extends BaseController {
	...
	public void list(String publishDate,String title,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (publishDate == null) publishDate = "";
		if (title == null) title = "";
		if(rows != 0)noticeService.setRows(rows);
		List<Notice> noticeList = noticeService.queryNotice(publishDate, title, page);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(publishDate, title);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String publishDate,String title,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (publishDate == null) publishDate = "";
		if (title == null) title = "";
		List<Notice> noticeList = noticeService.queryNotice(publishDate, title, currentPage);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(publishDate, title);
	    /*获取到总的页码数目*/
	    int totalPage = noticeService.getTotalPage();
	...
	public void OutToExcel(String publishDate,String title, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(publishDate == null) publishDate = "";
        if(title == null) title = "";
        List<Notice> noticeList = noticeService.queryNotice(publishDate,title);
